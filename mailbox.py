...
def fetch_recent_emails(limit: int = MAILBOX_FETCH_LIMIT, email_addr: str | None = None, password: str | None = None) -> List[Dict[str, Any]]:
    """Fetch the newest emails from the INBOX for display and persist them."""
    _email = email_addr or EMAIL
...
def get_mailbox_snapshot(limit: int = MAILBOX_FETCH_LIMIT, email_addr: str | None = None, password: str | None = None) -> List[Dict[str, Any]]:
    try:
        return fetch_recent_emails(limit, email_addr=email_addr, password=password)
    except Exception:
